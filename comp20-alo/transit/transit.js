...
function initialize() {
	...
	stationData = JSON.parse(stationList);
...
function renderMarkers(color) {
	...
	for(i = 0; i < stationData.length; i++) {
		if(stationData[i]["line"].toLowerCase() == color) {
			console.log(stationData[i]["line"] + ", " + stationData[i]["station"]);
			createMarker(i);
		}
	}
}
...
function createMarker(i) {
	var stationPos = new google.maps.LatLng(stationData[i]["lat"], stationData[i]["lng"]);
	var marker = new google.maps.Marker({
		position: stationPos,
		title: stationData[i]["station"],
		icon: tIcon,
	});
	marker.setMap(map);
